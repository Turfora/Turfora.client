...
import { getTurfById, updateTurf } from '../../../api/owner.api'
...
export default function EditTurfScreen({ navigation, route }: any) {
  ...
  const loadTurfData = async () => {
    try {
      ...
      console.error('[EditTurfScreen] Error loading turf:', error)
      Alert.alert('Error', 'Failed to load turf data')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handlePickImage = async () => {
    try {
      ...
      // Fixed: Use correct ImagePicker API
      ...
      console.error('[EditTurfScreen] Error picking image:', error)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error('[EditTurfScreen] Error:', error)
      Alert.alert('Error', error.response?.data?.message || 'Failed to update turf')
    } finally {
      setSubmitting(false)
    }
  }
        ...
        {existingImages.length > 0 && (
          <View style={styles.section}>
            <View style={styles.imagesHeader}>
              ...
              <Text style={styles.imageCount}>{existingImages.length}</Text>
            </View>
          ...
          >
            {submitting ? (
              ...
              <>
                ...
                <Text style={styles.submitButtonText}>Update Turf</Text>
              </>
            )}
          </TouchableOpacity>
